  ...
  describe("A list of functions") {
    ...
    it("can be parsed to a single PGFunction") {
      program("f(x) = { 2*x, 0 <= x <2") should parseAs(
        PGData(PGOptions(PGVariable("GraphDemo"),PGVariable("Graph"),PGVariable("x"),PGVariable("y"),PGVariable("docs/img/")),PGFunction(PGBounds(PGNumber(0.0),PGComparator("<="),PGVariable("x"),PGComparator("<"),PGNumber(2.0)),PGFunctionName("f"),PGVariable("x"),PGExpression(PGNumber(2.0),"*",PGVariable("x")),None)))
    }

    it("can parse exponents") {
      program("g(y) = { y^2, 0<y<4") should parseAs(
        PGData(PGOptions(PGVariable("GraphDemo"),PGVariable("Graph"),PGVariable("x"),PGVariable("y"),PGVariable("docs/img/")),PGFunction(PGBounds(PGNumber(0.0),PGComparator("<"),PGVariable("y"),PGComparator("<"),PGNumber(4.0)),PGFunctionName("g"),PGVariable("y"),PGExpression(PGVariable("y"),"^",PGNumber(2.0)),None)))
    }
